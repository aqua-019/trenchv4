...
import { getSolBalance, getTokenAccounts, dexTokens, dexPairs, getSolPrice, fetchSwaps, fetchTransfers, buildCostBasis, enrichTokens, buildSnapshot, buildHistoricalData, solanPayUrl, refreshTokenPrices, tokenTotalPnl, findBestTrade } from "@/lib/api";
...
function CardParticles({ active }) {
  ...
  return (<div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none", borderRadius: 12 }}>
    {Array.from({ length: 6 }, (_, i) => (<div key={i} style={{ position: "absolute", width: 3, height: 3, borderRadius: "50%", background: ["#9945ff","#00ff88","#14f195","#ffaa00","#7799ee","#ff4466"][i], left: `${15+i*14}%`, top: `${20+(i%3)*25}%`, opacity: 0.7, animation: `floatStar ${1.5+i*0.3}s ease-in-out infinite`, animationDelay: `${i*0.2}s` }} />))}
  </div>);
}

// FIX #1: Pie chart - shows SOL + individual token slices with labels
function Pie({ data, size = 260 }) {
  ...
  useEffect(() => {
    ...
    const cols = ["#9945ff","#00ff88","#ff4466","#ffaa00","#14f195","#7799ee","#ff6688","#44ddff","#ff9944","#aa66ff","#66ffcc","#ff44aa"];
    ...
    // Outer labels for slices > 5%
    pie(data).forEach((d, i) => {
      if (d.data.pct < 5) return;
      const mid = (d.startAngle + d.endAngle) / 2;
      const lx = Math.sin(mid) * (r + 18);
      const ly = -Math.cos(mid) * (r + 18);
      g.append("text").attr("x", lx).attr("y", ly).attr("text-anchor","middle").attr("dominant-baseline","central").attr("fill", col(i)).attr("font-size", 8).attr("font-weight", 800).text(d.data.label);
    });
...
// FIX #5: Portfolio chart - sharper, area fill, data dots, current value annotation
function PortfolioChart({ histData, tokens, solPrice, yMode }) {
  ...
  useEffect(() => {
    ...
    y.ticks(6).forEach(t => {
      svg.append("line").attr("x1",M.l).attr("x2",W-M.r).attr("y1",y(t)).attr("y2",y(t)).attr("stroke","#141e2e").attr("stroke-dasharray","2,3");
    ...
    x.ticks(Math.min(8, histData.length)).forEach(t => {
      svg.append("line").attr("x1",x(t)).attr("x2",x(t)).attr("y1",M.t).attr("y2",H-M.b).attr("stroke","#141e2e").attr("stroke-dasharray","1,4");
    ...
    // Legend
    [{ c:"#9945ff", l:"SOL" },{ c:"#ffaa00", l:"Tokens" },{ c:"#00ff88", l:"Total" }].forEach((e,i) => {
      svg.append("circle").attr("cx",M.l+i*70).attr("cy",M.t-10).attr("r",3).attr("fill",e.c);
      svg.append("text").attr("x",M.l+i*70+7).attr("y",M.t-7).attr("fill","#667788").attr("font-size",8).attr("font-weight",700).text(e.l);
    });
...
// FIX #6: PnL Distribution - top 5, $ticker labels, SOL values on bars
function PnlDistChart({ cb, tokens }) {
  ...
  useEffect(() => {
    ...
    svg.append("text").attr("x",W/2).attr("y",M.t-2).attr("text-anchor","middle").attr("fill","#556677").attr("font-size",7).text("Top 5 PnL (SOL)");
  }, [cb, tokens]);
...
function TokenModal({ token, cb, solPrice, totalPortfolio, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.88)", backdropFilter: "blur(20px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "linear-gradient(165deg, #0a1220, #0d1a2a 50%, #0a1018)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 18, width: "100%", maxWidth: 800, maxHeight: "94vh", overflow: "auto", boxShadow: "0 40px 80px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.04)" }}>
        ...
        {pairAddr && (<div style={{ margin: "12px 24px", borderRadius: 12, overflow: "hidden", border: "1px solid rgba(255,255,255,0.04)" }}><iframe src={`https://dexscreener.com/solana/${pairAddr}?embed=1&theme=dark&trades=0&info=0`} style={{ width: "100%", height: 320, border: "none" }} title="DexScreener" /></div>)}
        {c && (<div style={{ margin: "10px 24px", padding: "12px 16px", borderRadius: 12, background: totalPnlSol >= 0 ? "linear-gradient(135deg, rgba(0,255,136,0.06), rgba(153,69,255,0.04))" : "linear-gradient(135deg, rgba(255,68,102,0.06), rgba(153,69,255,0.04))", border: `1px solid ${totalPnlSol >= 0 ? "rgba(0,255,136,0.12)" : "rgba(255,68,102,0.12)"}` }}>
          ...
          {c.trades.length > 0 && (<div style={{ marginTop: 10 }}>
            ...
            <div style={{ maxHeight: 120, overflow: "auto" }}>{c.trades.slice(0, 20).map((t, i) => (
        ...
        {mp && (<div style={{ padding: "0 24px 18px", textAlign: "center" }}><a href={mp.url || `https://dexscreener.com/solana/${token.mint}`} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", background: "rgba(0,255,136,0.06)", border: "1px solid rgba(0,255,136,0.15)", color: "#00ff88", borderRadius: 8, padding: "7px 16px", fontSize: 10, fontWeight: 700, textDecoration: "none", textTransform: "uppercase" }}>DexScreener {"\u2192"}</a></div>)}
      </div>
    </div>
  );
}

// FIX #7: PnL Token List - ordered by USD holding value, shows $ticker, MCap, Vol, images from DexScreener
function PnlTokenList({ cb, tokens, solPrice, onSelectToken }) {
  const entries = useMemo(() => {
    return Object.entries(cb).map(([mint, c]) => {
      const token = tokens.find(t => t.mint === mint);
      const pnlSol = tokenTotalPnl(c, token);
      const holdingVal = token ? token.balance * (token.priceUsd || 0) : 0;
      return { mint, symbol: token?.symbol || null, name: token?.name || null, imageUrl: token?.imageUrl, pnlSol, pnlUsd: pnlSol * (solPrice || 0), buys: c.trades.filter(t => t.type === "BUY").length, sells: c.trades.filter(t => t.type === "SELL").length, solSpent: c.solSpent, solReceived: c.solReceived, marketCap: token?.marketCap || 0, volume24h: token?.volume24h || 0, holdingVal, token, balance: token?.balance || 0 };
    }).sort((a, b) => b.holdingVal - a.holdingVal);
  }, [cb, tokens, solPrice]);
  return (
    <div style={{ maxHeight: 400, overflow: "auto", marginTop: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "24px 28px 1.2fr 80px 80px 70px 60px 20px", padding: "6px 12px", borderBottom: "1px solid rgba(255,255,255,0.04)", fontSize: 7, color: "#445566", textTransform: "uppercase", letterSpacing: 1, fontWeight: 800 }}>
      ...
      {entries.map((e, i) => (
        <div key={e.mint} onClick={() => e.token && onSelectToken(e.token)} style={{ display: "grid", gridTemplateColumns: "24px 28px 1.2fr 80px 80px 70px 60px 20px", alignItems: "center", padding: "7px 12px", borderBottom: "1px solid rgba(255,255,255,0.02)", cursor: e.token ? "pointer" : "default", transition: "background 0.12s" }} onMouseEnter={ev => { ev.currentTarget.style.background = "rgba(153,69,255,0.05)"; }} onMouseLeave={ev => { ev.currentTarget.style.background = "transparent"; }}>
          <span style={{ fontSize: 9, color: "#445566", fontWeight: 800 }}>{i + 1}</span>
          {e.imageUrl ? <img src={e.imageUrl} alt="" style={{ width: 22, height: 22, borderRadius: 6 }} onError={ev => { ev.target.style.display = "none"; }} /> : <div style={{ width: 22, height: 22, borderRadius: 6, background: `hsl(${(e.mint.charCodeAt(0)*37)%360},45%,22%)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800, color: "#fff" }}>{(e.symbol||"?")[0]}</div>}
          <div style={{ minWidth: 0 }}><div style={{ fontSize: 10, fontWeight: 700, color: "#d0d8e8", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{e.symbol ? "$"+e.symbol : sAddr(e.mint)}</div><div style={{ fontSize: 7, color: "#3a4a5a", fontFamily: "monospace" }}>{sAddr(e.mint)}</div></div>
          <div style={{ textAlign: "right" }}><div style={{ fontSize: 10, fontWeight: 800, color: pCol(e.pnlSol) }}>{(e.pnlSol >= 0 ? "+" : "")+fmt(e.pnlSol, 3)} SOL</div><div style={{ fontSize: 7, color: "#556677" }}>{e.pnlUsd >= 0 ? "+" : ""}{fUsd(e.pnlUsd)}</div></div>
          ...
          <div style={{ textAlign: "center" }}><span style={{ fontSize: 9, color: "#00ff88", fontWeight: 700 }}>{e.buys}B</span><span style={{ fontSize: 8, color: "#334455" }}>/</span><span style={{ fontSize: 9, color: "#ff4466", fontWeight: 700 }}>{e.sells}S</span></div>
...
function TickerScroll({ tokens, solPrice }) {
  ...
  return (<div style={{ overflow: "hidden", background: "rgba(8,14,25,0.8)", border: "1px solid rgba(255,255,255,0.03)", borderRadius: 10, height: 36, position: "relative" }}>
    <div style={{ display: "flex", gap: 24, whiteSpace: "nowrap", animation: `tickerScroll ${items.length * 3}s linear infinite`, padding: "8px 0" }}>
      {items.map((t, i) => (<span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 10, flexShrink: 0 }}>
...
function CompareModal({ aq, gu, guWallet, guCb, aqCb, solPrice, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1100, background: "rgba(0,0,0,0.9)", backdropFilter: "blur(24px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "linear-gradient(170deg, #0c1525, #0a1220 50%, #080e1a)", border: "1px solid rgba(153,69,255,0.12)", borderRadius: 22, width: "100%", maxWidth: 680, maxHeight: "94vh", overflow: "auto", boxShadow: "0 40px 100px rgba(0,0,0,0.7)" }}>
        ...
        <div style={{ margin: "14px 26px", padding: "14px 18px", borderRadius: 14, textAlign: "center", background: guBeatsAq ? "linear-gradient(135deg, rgba(255,170,0,0.08), rgba(255,100,68,0.04))" : guIsAbysmal ? "linear-gradient(135deg, rgba(255,68,102,0.1), rgba(153,69,255,0.04))" : "linear-gradient(135deg, rgba(153,69,255,0.08), rgba(20,241,149,0.04))", border: `1px solid ${guBeatsAq ? "rgba(255,170,0,0.18)" : guIsAbysmal ? "rgba(255,68,102,0.18)" : "rgba(153,69,255,0.18)"}` }}>
          {guBeatsAq ? (<><div style={{ fontSize: 18, fontWeight: 900, color: "#ffaa00" }}>{"\uD83D\uDD25"} nice, you{"'"}re kicking aquatic{"'"}s ass.</div><div style={{ fontSize: 12, color: "#889988", marginTop: 4 }}>pray for him, or donate to his journey</div><div style={{ marginTop: 12, animation: "sparkle 2s ease-in-out infinite", fontSize: 13, fontWeight: 800, color: "#00ff88" }}>accepting 0.01-1 SOL donations</div><div style={{ fontFamily: "monospace", fontSize: 9, color: "#556677", marginTop: 4 }}>{AQUATIC_WALLET}</div><div style={{ marginTop: 10 }}><QRImg url={solanPayUrl(AQUATIC_WALLET)} size={140} /></div></>) : guIsAbysmal ? (<><div style={{ fontSize: 18, fontWeight: 900, color: "#ff4466" }}>you legit are awful</div><div style={{ marginTop: 12, background: "rgba(0,255,136,0.06)", border: "1px solid rgba(0,255,136,0.15)", borderRadius: 10, padding: "10px 16px", display: "inline-block" }}><div style={{ fontSize: 11, fontWeight: 700, color: "#00ff88" }}>accept .1 SOL and get back on your feet</div><div style={{ fontSize: 9, color: "#778899", marginTop: 6, textAlign: "left" }}>1. Tweet: <span style={{ color: "#44ddff" }}>@AquaticXCP hey! checked my PnL on your journey website, and I am so pathetic that I need 0.1 SOL sent to me. I sent you a DM!</span></div><div style={{ fontSize: 9, color: "#778899", marginTop: 4, textAlign: "left" }}>2. DM <span style={{ color: "#44ddff" }}>@AquaticXCP</span> on X</div><a href="https://x.com/intent/tweet?text=%40AquaticXCP%20hey!%20checked%20my%20PnL%20on%20your%20journey%20website%2C%20and%20I%20am%20so%20pathetic%20that%20I%20need%200.1%20SOL%20sent%20to%20me.%20I%20sent%20you%20a%20DM!" target="_blank" rel="noopener noreferrer" style={{ display: "inline-block", marginTop: 8, background: "rgba(29,155,240,0.1)", border: "1px solid rgba(29,155,240,0.3)", color: "#1d9bf0", borderRadius: 8, padding: "7px 14px", fontSize: 10, fontWeight: 700, textDecoration: "none" }}>Tweet @AquaticXCP</a></div></>) : (<><div style={{ fontSize: 16, fontWeight: 900, color: "#9945ff" }}>by some stroke of chance, you are accurately worse than aquatic is at trenching.</div><div style={{ fontSize: 12, color: "#778899", marginTop: 6 }}>study the greats..</div></>)}
        </div>
        <div style={{ padding: "0 26px 22px" }}>
          {metrics.map((m, i) => { const mx = Math.max(Math.abs(m.aq||0.01), Math.abs(m.gu||0.01)); return (<div key={i} style={{ marginBottom: 12 }}><div style={{ fontSize: 9, color: "#667788", fontWeight: 700, marginBottom: 4 }}>{m.label}</div><div style={{ display: "flex", gap: 6, alignItems: "center" }}><div style={{ flex: 1 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}><span style={{ fontSize: 8, color: "#9945ff", fontWeight: 700 }}>AQ</span><span style={{ fontSize: 10, color: "#c8d0e0", fontWeight: 800 }}>{m.f(m.aq)}</span></div><div style={{ height: 6, background: "rgba(255,255,255,0.03)", borderRadius: 4, overflow: "hidden" }}><div style={{ height: "100%", width: (Math.abs(m.aq||0)/mx)*100+"%", background: "linear-gradient(90deg, #9945ff, #14f195)", borderRadius: 4 }} /></div></div><div style={{ width: 14, textAlign: "center", color: "#334455", fontSize: 8, fontWeight: 800 }}>v</div><div style={{ flex: 1 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}><span style={{ fontSize: 8, color: "#ffaa00", fontWeight: 700 }}>YOU</span><span style={{ fontSize: 10, color: "#c8d0e0", fontWeight: 800 }}>{m.f(m.gu)}</span></div><div style={{ height: 6, background: "rgba(255,255,255,0.03)", borderRadius: 4, overflow: "hidden" }}><div style={{ height: "100%", width: (Math.abs(m.gu||0)/mx)*100+"%", background: "linear-gradient(90deg, #ffaa00, #ff6644)", borderRadius: 4 }} /></div></div></div></div>); })}
        </div>
      </div>
    </div>
  );
...
function Row({ token, idx, cb, sp, onClick }) {
  ...
  const c = cb?.[token.mint];
  const pnlSol = c ? tokenTotalPnl(c, token) : null;
...
export default function Dashboard() {
  ...
  // FIX #4: Refresh token prices every 5 minutes
  useEffect(() => {
    const iv = setInterval(async () => {
      if (tokens.length > 0) {
        try { const r = await refreshTokenPrices(tokens); setTokens(r); } catch (e) { console.warn("Price refresh:", e); }
      }
    }, 5 * 60 * 1000);
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 1200, margin: "0 auto", padding: "0 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 16 }}>
          {[
            ...
            { l: "Token Value", v: loading ? "..." : fUsd(tokVal), s: tokens.length+" tokens \u00B7 auto-refresh 5m", c: "#ffaa44", g: "rgba(255,170,0,0.08)" },
        ...
        {Object.keys(cb).length > 0 && (
          <div style={{ marginBottom: 16, borderRadius: 14, background: "linear-gradient(135deg, rgba(153,69,255,0.05), rgba(0,255,136,0.03))", border: "1px solid rgba(153,69,255,0.1)", overflow: "hidden" }}>
            <div onClick={() => setPnlOpen(!pnlOpen)} style={{ padding: "16px 20px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", cursor: "pointer", transition: "background 0.12s" }} onMouseEnter={e => { e.currentTarget.style.background = "rgba(153,69,255,0.04)"; }} onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}>
              ...
              <span style={{ fontSize: 13, color: "#c8d0e0", fontWeight: 700 }}>{Object.keys(cb).length} tokens {pnlOpen ? "\u25B4" : "\u25BE"}</span>
              <span style={{ fontSize: 11, color: "#556677" }}>{"\u00B7"}</span>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 22 }}>
          <div>
            ...
            <div style={{ background: "rgba(8,14,25,0.6)", border: "1px solid rgba(255,255,255,0.03)", borderRadius: 14, overflow: "hidden" }}>
              <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(255,255,255,0.04)", fontSize: 9, color: "#9977cc", textTransform: "uppercase", letterSpacing: 1.5, fontWeight: 700 }}>Holdings ({tokens.length + 1} assets)</div>
              <div style={{ maxHeight: 440, overflow: "auto" }}>
                {solBal != null && (<div style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 14px", borderBottom: "1px solid rgba(255,255,255,0.025)", background: "rgba(153,69,255,0.03)" }}>
                ...
                {tokensByValue.map((t, i) => { const val = t.balance * (t.priceUsd || 0); return (
                  <div key={t.mint} onClick={() => setSelToken(t)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", borderBottom: "1px solid rgba(255,255,255,0.02)", cursor: "pointer", transition: "background 0.1s" }} onMouseEnter={e => { e.currentTarget.style.background = "rgba(0,255,136,0.02)"; }} onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}>
                    <span style={{ fontSize: 9, color: "#334455", fontWeight: 800, width: 20 }}>{i + 2}</span>
                    {t.imageUrl ? <img src={t.imageUrl} alt="" style={{ width: 26, height: 26, borderRadius: 7 }} onError={e => { e.target.style.display = "none"; }} /> : <div style={{ width: 26, height: 26, borderRadius: 7, background: `hsl(${(t.mint.charCodeAt(0)*37)%360},45%,22%)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, color: "#fff" }}>{(t.symbol||"?")[0]}</div>}
                    ...
                    <div style={{ textAlign: "right" }}><div style={{ fontSize: 11, fontWeight: 800, color: val > 0 ? "#e0e8f0" : "#3a4a5a" }}>{val > 0 ? fUsd(val) : "$0"}</div><div style={{ fontSize: 8, color: "#556677" }}>{fmt(t.balance, t.balance < 1 ? 4 : 2)}</div></div>
                  </div>); })}
              </div>
            </div>
          </div>
          <div>
            ...
            <div style={{ background: "rgba(8,14,25,0.6)", border: "1px solid rgba(255,255,255,0.03)", borderRadius: 14, padding: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <div><div style={{ fontSize: 9, color: "#9977cc", textTransform: "uppercase", letterSpacing: 1.5, fontWeight: 700 }}>Wallet Performance</div><div style={{ fontSize: 8, color: "#445566", marginTop: 2 }}>From 2/6/26 11PM EST {"\u2192"} Now</div></div>
                <div style={{ display: "flex", gap: 4 }}>
                  {["sol","usd"].map(m => (<button key={m} onClick={() => setChartYMode(m)} style={{ background: chartYMode === m ? "rgba(0,255,136,0.1)" : "rgba(255,255,255,0.02)", border: `1px solid ${chartYMode === m ? "rgba(0,255,136,0.2)" : "rgba(255,255,255,0.04)"}`, color: chartYMode === m ? "#00ff88" : "#556677", padding: "3px 8px", borderRadius: 5, fontSize: 9, fontWeight: 700, cursor: "pointer", textTransform: "uppercase" }}>{m}</button>))}
                </div>
              </div>
        ...
        <footer style={{ padding: "12px 0 32px", textAlign: "center", fontSize: 9, color: "#2a3a4a" }}>
          ...
          <div style={{ fontFamily: "monospace", marginTop: 3 }}>v6 {"\u00B7"} Trenches Edition</div>
        </footer>
      </div>
